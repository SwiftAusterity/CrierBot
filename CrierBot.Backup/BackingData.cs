...
namespace NetMud.Backup
{
    ...
    public static class BackingData
    {
        ...
        public static bool WriteFullBackup()
        {
            ...
            try
            {
                ...
                foreach (var entity in entities)
                    fileAccessor.WriteEntity(entity as IStoredData);
        ...
        {
            var implimentedTypes = typeof(BackingDataPartial).Assembly.GetTypes().Where(ty => ty.GetInterfaces().Contains(typeof(IStoredData))
                                                                                && ty.IsClass
                                                                                && !ty.IsAbstract);
        ...
        {
            if (!objectType.GetInterfaces().Contains(typeof(IStoredData)))
                return false;
