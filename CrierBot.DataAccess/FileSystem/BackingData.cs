...
namespace NetMud.DataAccess.FileSystem
{
    public class BackingData : FileAccessor
    {
        ...
        public IStoredData ReadEntity(FileInfo file, Type entityType)
        {
            var fileData = ReadFile(file);
            var blankEntity = Activator.CreateInstance(entityType) as IStoredData;

            return blankEntity.FromBytes(fileData) as IStoredData;
        }
        ...
        public void WriteEntity(IStoredData entity)
        {
        ...
        public void ArchiveEntity(IStoredData entity)
        {
            var dirName = BaseDirectory + CurrentDirectoryName + entity.GetType().Name + "/";
        ...
        private string GetEntityFilename(IStoredData entity)
        {
